...
## Overview
FacePic uses a hybrid "Personal Cloud" architecture where heavy image processing (face detection) happens locally on your machine, while storage and metadata are managed in the cloud via a Cloudflare Worker (Backend). This ensures privacy, reduces cloud compute costs, and leverages local hardware.

## Components

### 1. Local Environment (Docker)
- **Container**: `facepic-processor` (Python 3.12)
- **Script**: `processor/process_images.py`
...
### 2. Cloud Backend (Cloudflare Worker)
- **Folder**: `backend/`
- **Framework**: Hono
- **Database**: MongoDB Atlas (via Mongoose)
- **Storage**: Cloudflare R2
- **Role**: Serves as the primary API for the frontend, handles image uploads (presigned URLs), and manages metadata.
...
├── backend/                # [Cloud] Cloudflare Worker API
│   ├── src/
│   └── wrangler.toml
