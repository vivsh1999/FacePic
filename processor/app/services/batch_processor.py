...
def find_matching_person_optimized(face_encoding, threshold=None):
    ...
    if threshold is None:
        # Convert distance tolerance to similarity threshold
        # InsightFace tolerance is distance (1 - similarity)
        # So similarity threshold = 1 - tolerance
        threshold = 1 - settings.insightface_tolerance
...
def process_upload_task(task_data, import_dir, thumbnail_dir):
    image_id, relative_path, unique_filename, mime_type, thumb_filename = task_data
    
    try:
        storage = get_storage_service()
        
        file_path = os.path.join(import_dir, relative_path)
        ...
        # Handle thumbnail
        local_thumb_path = os.path.join(thumbnail_dir, thumb_filename)
        
        if os.path.exists(local_thumb_path):
            with open(local_thumb_path, 'rb') as f:
                storage.upload_fileobj(f, thumb_filename, "image/jpeg")
        else:
            # Recreate thumbnail
            img = Image.open(io.BytesIO(image_bytes))
            thumb_io = create_thumbnail(img)
        ...
        # Return success and data for DB update
        return True, {
            "_id": image_id,
...
def upload_worker_loop(task_queue, result_queue, import_dir, thumbnail_dir):
    signal.signal(signal.SIGINT, signal.SIG_IGN)
    while True:
        try:
            task = task_queue.get(timeout=1.0)
            if task is None:
                break
            success, result = process_upload_task(task, import_dir, thumbnail_dir)
...
class BatchProcessor:
    ...
    def process_pending_uploads(self):
        ...
        for image in pending_images:
            ...
            tasks.append((image["_id"], relative_path, unique_filename, mime_type, thumb_filename))
            
        if not tasks:
            print("No valid pending uploads found (missing paths).")
            return

        # Parallel Processing
        MAX_CPU_CORES = multiprocessing.cpu_count()
        # Uploads are I/O bound, so we can use more workers than cores if network allows,
        # but let's stick to a reasonable number to avoid choking the network/DB.
        NUM_WORKERS = min(16, MAX_CPU_CORES * 2) 
        
        print(f"Starting upload with {NUM_WORKERS} workers...")
        ...
        for _ in range(NUM_WORKERS):
            p = ctx.Process(
                ...
                args=(task_queue, result_queue, self.import_dir, settings.thumbnail_dir)
            )
            p.start()
            workers.append(p)
        ...
        pbar = tqdm(total=len(tasks), desc=f"Uploading [{NUM_WORKERS} workers]", unit="img")
        ...
        try:
            ...
            while processed_count < len(tasks):
                try:
                    success, result = result_queue.get(timeout=0.5)
                    processed_count += 1
                    pbar.update(1)
                    ...
                    if success:
                        ...
                        if len(bulk_ops) >= BATCH_SIZE:
                            self.db.images.bulk_write(bulk_ops)
                            bulk_ops = []
                    else:
                        errors += 1
                    ...
                    if not any(p.is_alive() for p in workers):
                        pbar.write("All workers died unexpectedly.")
                        break
                    continue
            ...
            pbar.write("\nStopping uploads...")
            # Process remaining ops before exit if any
            if bulk_ops:
                try:
                    self.db.images.bulk_write(bulk_ops)
                except:
        ...
        print(f"Uploaded {count} pending images. Errors: {errors}")
